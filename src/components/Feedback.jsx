...
function Feedback() {
  ...
  return (
    <section
      id="feedback"
      className="w-full flex justify-center items-center py-24 bg-[#F1FAEE]"
    >
      <div className="w-full max-w-2xl mx-4 bg-white border border-slate-200 rounded-[2.5rem] shadow-xl shadow-blue-900/5 p-10 relative overflow-hidden">
        ...
        <h2 className="text-4xl font-black text-[#1D3557] text-center tracking-tight">
        ...
        {message && (
          <p
            className={`${message === 'Invalid email' ? 'text-red-500 bg-red-50' : 'text-emerald-600 bg-emerald-50'} text-center mt-6 font-bold py-3 rounded-xl border border-transparent`}
          >
            {message}
          </p>
        )}
        ...
        <form className="mt-10 space-y-6" onSubmit={handleSubmit}>
          <div>
            <label className="block text-sm font-bold text-[#1D3557] mb-2 uppercase tracking-wide">
              Your Name
            </label>
            <input
              type="text"
