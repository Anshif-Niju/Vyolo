              ...
              >
                ...
                <div className="space-y-3">
                  <div className="bg-slate-900/50 rounded-xl p-4">
                    <p className="text-xs font-bold text-slate-400 mb-2 uppercase">Items</p>
                    {order.product && order.product.map((item, index) => (
                      <div key={index} className="flex justify-between text-sm py-1 border-b border-slate-700 last:border-0">
                        <span className="text-slate-200">{item.name}</span>
                        <span className="text-cyan-400 font-medium">Size: {item.size}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-2">
                    <div>
                      ...
                      <p className="text-xl font-bold text-white">
                        ${order.product.reduce((total, item) => total + (item.price * item.size), 0)}
                      </p>
                    </div>
